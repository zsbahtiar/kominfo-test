...
export default function DashboardPage() {
    ...
    const [genderStats, setGenderStats] = useState({ jumlah_laki_laki: 0, jumlah_perempuan: 0 })
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const employeeResponse = await fetch('https://seleksi-tenagait.tasikmalayakab.go.id/api/pegawai', {
                    headers: {
                ...
                const genderResponse = await fetch('https://seleksi-tenagait.tasikmalayakab.go.id/api/pegawai/jumlah/jeniskelamin', {
                    headers: {
                        'x-api-pegawai': 'TestSeleksiPegawai2025'
                    }
                })

                if (!employeeResponse.ok || !genderResponse.ok) {
                    throw new Error('Failed to fetch data')
                }

                const [employeeData, genderData] = await Promise.all([
                    employeeResponse.json(),
                    genderResponse.json()
                ])

                setPegawaiList(employeeData)
